...
export class RoleAssignComponent implements OnInit, OnDestroy {
  ...
  // }
  ngOnInit() {
  const userId = this.config.data?.id;   // 👈 DÒNG QUAN TRỌNG
  ...
  forkJoin({ roles })
    ...
    .subscribe({
      next: (response: any) => {
        ...
        roles.forEach(r => {
          if (r.name) this.availableRoles.push(r.name);
        });

        this.loadDetail(userId); // ✅ CHỈ DÙNG userId
        this.toggleBlockUI(false);
      },
  ...
  if (!id) {
    console.error('❌ loadDetail: id rỗng');
    return;
  }
